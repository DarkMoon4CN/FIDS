/// <reference path="baggageClaim.js" />
///  FOR  baggageClaim
//中英文切换变量
var changed = 0;
//切换时间
...
var BaggageClaim =
    {
        Init: function () { BaggageClaim.ListInit(); }
    }
BaggageClaim.ListInit = function ()
{
    $(".menu-text").on("click", function () {
        BaggageClaim.CreateLabel();
    });

    $(".menu-pic").on("click", function () {
        BaggageClaim.CreateImage();
    });
    $(".menu-time").on("click", function () {
        BaggageClaim.CreateTime();
    });
...
BaggageClaim.CreateBgColor = function () {
    //背景加在body上
    tag = "9";
...
BaggageClaim.CreateLabel = function () {
    //创建label
    tag = "1";
    var url = "/Template/GetGUID";
    var data = {};
...
BaggageClaim.CreateImage = function () {
    //创建image
    tag = "2";
    var url = "/Template/GetGUID";
    var data = {};
...
BaggageClaim.CreateTime = function () {
    //创建时钟
    tag = "3";
    var url = "/Template/GetGUID";
    var data = {};
...
BaggageClaim.Submit = function () {
    ...
    var url = "/Template/DoAddBaggageClaimTemplate";
    ...
    if (Global.PostData.Status== 1) {
        layer.confirm('组件已保存完毕,查看预览效果？', { icon: 1, title: '提示', time: 5000 }
            , function (index) {
                window.open("/Actual/BaggageClaim");
            });
    } else {
        layer.msg(_defaultData.Message, { time: 1000 });
    }
}
BaggageClaim.Reset = function ()
{
    layer.confirm('行李提取编辑模块恢复出厂设置？', { icon: 3, title: '确认' }
    , function (index) {
        var url = "/Template/Reset";
        var data = { ak: "BaggageClaim" };
