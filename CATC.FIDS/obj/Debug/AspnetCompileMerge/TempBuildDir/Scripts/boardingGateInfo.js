/// <reference path="boardingGateInfo.js" />
///  FOR  boardingGateInfo
//中英文切换变量
var changed = 0;
//切换时间
var changeTime = 5;
...
var BoardingGateInfo =
    {
        Init: function () { BoardingGateInfo.ListInit(); }
    }
BoardingGateInfo.ListInit = function () {
    $(".menu-text").on("click", function () {
        BoardingGateInfo.CreateLabel();
    });

    $(".menu-pic").on("click", function () {
        BoardingGateInfo.CreateImage();
    });
    $(".menu-time").on("click", function () {
        BoardingGateInfo.CreateTime();
    });
    $(".menu-save").on("click", function () {
        BoardingGateInfo.Submit();
    });
    $(".menu-clear").on("click", function () {
        BoardingGateInfo.Reset();
    });
    $(".menu-bg").on("click", function () {
        BoardingGateInfo.CreateBgColor();
    });
}
BoardingGateInfo.CreateBgColor = function () {
    //背景加在body上
    tag = "9";
...
BoardingGateInfo.CreateLabel = function () {
    //创建label
    tag = "1";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGateInfo.CreateImage = function () {
    //创建image
    tag = "2";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGateInfo.CreateTime = function () {
    //创建时钟
    tag = "3";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGateInfo.Submit = function () {
    ...
    var url = "/Template/DoAddBoardingGateInfoTemplate";
    ...
    if (Global.PostData.Status == 1) {
        layer.confirm('组件已保存完毕,查看预览效果？', { icon: 1, title: '提示', time: 5000 }
            , function (index) {
                window.open("/Actual/BoardingGateInfo");
            });
    } else {
        layer.msg(Global.PostData.Message, { time: 1000 });
    }
}
BoardingGateInfo.Reset = function () {
    layer.confirm('登机口横版(无气象)模块恢复出厂设置？', { icon: 3, title: '确认' }
    , function (index) {
        var url = "/Template/Reset";
        var data = { ak: "BoardingGateInfo" };
