/// <reference path="BoardingGuide.js" />
///  FOR  BoardingGuide
//中英文切换变量
var changed = 0;
//切换时间
...
var BoardingGuide =
    {
        Init: function () { BoardingGuide.ListInit(); }
    }
BoardingGuide.ListInit = function () {
    $(".menu-text").on("click", function () {
        BoardingGuide.CreateLabel();
    });

    $(".menu-pic").on("click", function () {
        BoardingGuide.CreateImage();
    });
    $(".menu-time").on("click", function () {
        BoardingGuide.CreateTime();
    });
    $(".menu-save").on("click", function () {
        BoardingGuide.Submit();
    });
    $(".menu-clear").on("click", function () {
        BoardingGuide.Reset();
    });
    $(".menu-bg").on("click", function () {
        BoardingGuide.CreateBgColor();
    });
}
BoardingGuide.CreateBgColor = function () {
    //背景加在body上
    tag = "9";
...
BoardingGuide.CreateLabel = function () {
    //创建label
    tag = "1";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGuide.CreateImage = function () {
    //创建image
    tag = "2";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGuide.CreateTime = function () {
    //创建时钟
    tag = "3";
    var url = "/Template/GetGUID";
    var data = {};
...
BoardingGuide.Submit = function () {
    ...
    var url = "/Template/DoAddBoardingGuideTemplate";
    ...
    if (Global.PostData.Status == 1) {
        layer.confirm('组件已保存完毕,查看预览效果？', { icon: 1, title: '提示', time: 5000 }
            , function (index) {
                window.open("/Actual/BoardingGuide");
            });
    } else {
        layer.msg(Global.PostData.Message, { time: 1000 });
    }
}
BoardingGuide.Reset = function () {
    layer.confirm('登机引导编辑模块恢复出厂设置？', { icon: 3, title: '确认' }
    , function (index) {
        var url = "/Template/Reset";
        var data = { ak: "BoardingGuide" };
