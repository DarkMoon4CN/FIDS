/// <reference path="VNWBoardingGuide.js" />
///  FOR  VNWBoardingGuide
//中英文切换变量
var changed = 0;
//切换时间
var changeTime = 5;
...
var VNWBoardingGuide =
    {
        Init: function () { VNWBoardingGuide.ListInit(); }
    }
VNWBoardingGuide.ListInit = function () {
    $(".menu-text").on("click", function () {
        VNWBoardingGuide.CreateLabel();
    });

    $(".menu-pic").on("click", function () {
        VNWBoardingGuide.CreateImage();
    });
    $(".menu-time").on("click", function () {
        VNWBoardingGuide.CreateTime();
    });
    $(".menu-save").on("click", function () {
        VNWBoardingGuide.Submit();
    });
    $(".menu-clear").on("click", function () {
        VNWBoardingGuide.Reset();
    });
    $(".menu-bg").on("click", function () {
        VNWBoardingGuide.CreateBgColor();
    });
}
VNWBoardingGuide.CreateBgColor = function () {
    //背景加在body上
    tag = "9";
...
VNWBoardingGuide.CreateLabel = function () {
    //创建label
    tag = "1";
    var url = "/Template/GetGUID";
    var data = {};
...
VNWBoardingGuide.CreateImage = function () {
    //创建image
    tag = "2";
    var url = "/Template/GetGUID";
    var data = {};
...
VNWBoardingGuide.CreateTime = function () {
    //创建时钟
    tag = "3";
    var url = "/Template/GetGUID";
    var data = {};
...
VNWBoardingGuide.Submit = function () {
    ...
    var url = "/Template/DoAddVNWBoardingGateTemplate";
    ...
    if (Global.PostData.Status == 1) {
        layer.confirm('组件已保存完毕,查看预览效果？', { icon: 1, title: '提示', time: 5000 }
            , function (index) {
                window.open("/Actual/VNWBoardingGate");
            });
    } else {
        layer.msg(Global.PostData.Message, { time: 1000 });
    }
}
VNWBoardingGuide.Reset = function () {
    layer.confirm('登机口(竖版、无气象)模块恢复出厂设置？', { icon: 3, title: '确认' }
    , function (index) {
        var url = "/Template/Reset";
        var data = { ak: "VNWBoardingGate" };
